...
**NAS + Graph RAG + 로컬 LLM 채팅**. Mac mini M4(32GB)에서 Docker Compose 로 돌리는
다중 사용자 웹서비스다. 사용자마다 저장소와 지식 그래프가 완전히 분리된다. 외부로 열리는 포트는 **3040** 하나뿐이다.
...
- **채팅** — SSE 스트리밍, 페르소나(시스템 프롬프트+모델+temperature), 세션별 RAG 토글,
  ...
  모델은 로컬(Ollama) 외에 **DeepSeek · OpenAI · Claude(Anthropic)** API 중에 고른다.
...
- **웹 · 논문 검색** — 채팅에서 토글 하나로 켠다. 논문은 OpenAlex·Crossref·arXiv,
  ...
  한글 질문은 영어 주제어로 바꿔 논문 DB 에 던진다. RAG 와 동시에 켤 수 있다
- **숨김 폴더** — 목록에서 제외되고 하위로 상속. RAG 색인에서도 빠진다
- **파일 잠금** — 열람 시 비밀번호 요구(`423`). 해제는 N분간 유지
- **설정 페이지** — 연결·모델·RAG·NAS·보안·페르소나 탭.
  모델과 외부 API·OCR·계정은 **관리자만** 보이고, 일반 사용자에게는 자기 대화와
  RAG·NAS·보안 설정만 열린다
...
옵시디언 볼트를 iCloud 로 내보내려면 `.env` 에 아래를 넣고 `make restart` 한다.
...
- **웹·논문 검색은 공개 API 만 쓴다.** 그래서 무료지만 한계도 있다. OpenAlex 는 연락처
  없이 부르면 `429`(속도 제한)를 맞기 쉬우니 설정 → RAG → 웹·논문 검색에 이메일을
  넣어두면 좋다(polite pool). DuckDuckGo 는 HTML 을 긁는 방식이라 막히면 논문 결과만
  나온다. 한 소스가 죽어도 나머지로 계속 진행한다.
